...
class CategoryAwareAttributePredictor(nn.Module):
    def __init__(self, clip_dim=512, category_attributes=None, attribute_dims=None, hidden_dim=512, dropout_rate=0.2, num_hidden_layers=1):
        ...
        self.category_predictors = nn.ModuleDict()
        
        for category, attributes in category_attributes.items():
            layers = []    
            # Input layer
            layers.append(nn.Linear(clip_dim, 768))
            layers.append(nn.LayerNorm(768))
            layers.append(nn.ReLU())
            # layers.append(nn.Dropout(dropout_rate))
            self.category_predictors[category] = nn.Sequential(*layers)
            ...
            for attr_name in attributes.keys():
                ...
                if key in attribute_dims:
                    ...
                    # Input layer
                    layers.append(nn.Linear(768, hidden_dim))
                    layers.append(nn.ReLU())
                    layers.append(nn.Dropout(dropout_rate))
    ...
    def forward(self, clip_features, category):
        ...
        clip_features = self.category_predictors[category](clip_features)
...
#         # Track attribute keys and create parallel layers
#         self.category_to_attr_keys = {cat: [] for cat in category_attributes.keys()}
#         self.parallel_predictors = nn.ModuleDict()
        
#         for category, attributes in category_attributes.items():
#             # Category predictor remains the same
#             layers = []    
#             layers.append(nn.Linear(clip_dim, 768))
#             layers.append(nn.LayerNorm(768))
#             layers.append(nn.ReLU())
...
#             # Create parallel layers for each category
#             attr_keys = []
#             output_dims = []
...
#                     output_dims.append(attribute_dims[key])
            
#             if attr_keys:
#                 self.category_to_attr_keys[category] = attr_keys
#                 total_output_dim = sum(output_dims)
                
#                 parallel_layers = []
#                 # Input layer
#                 parallel_layers.append(nn.Linear(768, hidden_dim))
#                 parallel_layers.append(nn.ReLU())
#                 parallel_layers.append(nn.Dropout(dropout_rate))
                
#                 # Additional hidden layers
#                 for _ in range(num_hidden_layers - 1):
#                     parallel_layers.append(nn.Linear(hidden_dim, hidden_dim))
#                     parallel_layers.append(nn.ReLU())
#                     parallel_layers.append(nn.Dropout(dropout_rate))
                
#                 # Single output layer that predicts all attributes at once
#                 parallel_layers.append(nn.Linear(hidden_dim, total_output_dim))
                
#                 self.parallel_predictors[category] = nn.Sequential(*parallel_layers)
                
#                 # Store output dimensions for splitting
#                 self.output_dims = {category: output_dims}
    
#     def forward(self, clip_features, category):
#         results = {}
        
#         # Convert clip features to float and apply category-specific transformation
#         transformed_features = self.category_predictors[category](clip_features)
        
#         # Get attribute keys for this category
#         attr_keys = self.category_to_attr_keys[category]
        
#         if attr_keys:
#             # Process all attributes in a single forward pass
#             all_predictions = self.parallel_predictors[category](transformed_features)
            
#             # Split the predictions along the feature dimension
#             split_predictions = torch.split(all_predictions, self.output_dims[category], dim=-1)
            
#             # Create results dictionary
#             results = {key: pred for key, pred in zip(attr_keys, split_predictions)}
...
def train_model_with_validation(model, train_loader, val_loader, device, train_dataset, 
    ...
    optimizer = optim.AdamW([
        ...
        # *[{'params': m.parameters(), 'lr': predictor_lr, 'weight_decay': weight_decay} for m in model.category_predictors.values()],
        ...
        for epoch in epoch_pbar:
            ...
            for batch_idx, (images, categories, targets) in enumerate(train_pbar):
                try:
                    ...
                    category_masks = {category: [c == category for c in categories] for category in category_class_attribute_mapping.keys()}
                    for category, mask in category_masks.items():
                        predictions = model(features[mask], category) 
                        for key, pred in predictions.items():
                            targets_cat = targets[key][mask]
                            target_val_mask = (targets_cat != -1) 
                            pred = pred[target_val_mask]
                            
                            target_tensor = targets_cat[target_val_mask].to(device)
                            loss = criterion(pred, target_tensor)
                        
                            batch_loss += loss
                                
                            _, predicted = torch.max(pred, 1)
                            is_correct = (predicted == target_tensor).sum().item()
                            num = pred.size(0)
                            attr_correct[key] += is_correct
                            attr_total[key] += num
            ...
            if val_acc > best_val_accuracy:
                ...
                save_path = os.path.join(checkpoint_dir, f'ht_connect_bgremove_indoptim_cnet_{epoch+1}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth')
...
def main():
    ...
    try:
        ...
        for hidden_dim, dropout_rate, num_hidden_layers in product(
            ...
            # connector_mlp = ConnectorMLP(connect_dim=768+2048, clip_dim=512, num_hidden_layers=1, logger=logger).to(device)
            connector_mlp = None
