...
def validate_model(model, clip_model, val_loader, criterion, device, logger):
    """
    Validate the model on validation set with SareeCNN support
    """
    model.eval()
    clip_model.eval()
    val_loss = 0.0
    ...
    with torch.no_grad():
        for images, categories, targets in val_loader:
            ...
            for i in range(batch_size):
                ...
                # For Sarees, pass both CLIP features and original image
                if category == 'Sarees':
                    predictions = model((img_features, images[i:i+1]), category)
                else:
...
class ProductDataset(Dataset):
    def __init__(self, csv_path, image_dir, train=True):
        ...
        self.clip_model, self.base_preprocess = clip.load("ViT-L/14", device="cuda")
...
class SareeCNNModule(nn.Module):
    def __init__(self, output_dim=512):
        super(SareeCNNModule, self).__init__()
        
        # Use EfficientNet-B0 as the base CNN
        efficientnet = models.efficientnet_b0(pretrained=True)
        
        # Remove the original classifier
        self.features = nn.Sequential(*list(efficientnet.children())[:-1])
        
        # Add spatial attention modules for different saree regions
        self.border_attention = nn.Sequential(
            nn.Conv2d(1280, 1, kernel_size=1),
            nn.Sigmoid()
        )
        
        self.pallu_attention = nn.Sequential(
            nn.Conv2d(1280, 1, kernel_size=1),
            nn.Sigmoid()
        )
        
        self.body_attention = nn.Sequential(
            nn.Conv2d(1280, 1, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        border_weights = self.border_attention(features)
        pallu_weights = self.pallu_attention(features)
        body_weights = self.body_attention(features)
        
        # Combine attended features
        attended_features = features * (border_weights + pallu_weights + body_weights)
        
        # Process and return final features
        return self.post_attention(attended_features)
...
class CategoryAwareAttributePredictor(nn.Module):
    def __init__(self, clip_dim=512, category_attributes=None, attribute_dims=None, hidden_dim=512, dropout_rate=0.2, num_hidden_layers=1):
        ...
        # Add Saree-specific CNN module
        self.saree_cnn = SareeCNNModule(output_dim=hidden_dim)
        
        # Feature fusion layer for Saree category
        self.saree_fusion = nn.Sequential(
            nn.Linear(clip_dim + hidden_dim, hidden_dim),
            nn.ReLU(),
        ...
        for category, attributes in category_attributes.items():
            for attr_name in attributes.keys():
                ...
                if key in attribute_dims:
                    ...
                    # Input layer - different for Sarees vs other categories
                    if category == 'Sarees':
    ...
    def forward(self, x, category):
        ...
        # Convert CLIP features to float
        if isinstance(x, tuple):  # If input includes original image
            clip_features, image = x
            clip_features = clip_features.float()
        else:  # If input is just CLIP features
            clip_features = x.float()
            image = None
        
        # Special processing for Sarees
        if category == 'Sarees' and image is not None:
            # Get CNN features
            cnn_features = self.saree_cnn(image)
            
            # Fuse CLIP and CNN features
            fused_features = self.saree_fusion(
                torch.cat([clip_features, cnn_features], dim=1)
            )
            ...
            for attr_name in category_attrs.keys():
                ...
                if key in self.attribute_predictors:
                    results[key] = self.attribute_predictors[key](fused_features)
        
        else:  # For other categories, use CLIP features directly
            for attr_name in category_attrs.keys():
                key = f"{category}_{attr_name}"
                if key in self.attribute_predictors:
...
def setup_logging(log_dir="logs_e40"):
    ...
    log_file = os.path.join(log_dir, f'train_sc_{timestamp}.log')
...
def train_model_with_validation(model, train_loader, val_loader, device, train_dataset, 
    ...
    """
    Modified training function with validation and SareeCNN support
    """
    if logger is None:
    ...
    # Create separate parameter groups to ensure no overlap
    clip_params = list(clip_model.parameters())
    predictor_params = list(model.parameters())
    saree_cnn_params = list(model.saree_cnn.parameters())
    
    # Remove any potential overlap between predictor_params and saree_cnn_params
    predictor_params = [p for p in predictor_params if not any(p is p2 for p2 in saree_cnn_params)]
    
    # Create optimizer with distinct parameter groups
    optimizer = optim.AdamW([
        {'params': clip_params, 'lr': clip_lr},
        {'params': predictor_params, 'lr': predictor_lr, 'weight_decay': weight_decay},
        {'params': saree_cnn_params, 'lr': predictor_lr, 'weight_decay': weight_decay}
    ], betas=(beta1, beta2))
    ...
    try:
        for epoch in epoch_pbar:
            ...
            for batch_idx, (images, categories, targets) in enumerate(train_pbar):
                try:
                    ...
                    if valid_predictions > 0:
                        ...
                        torch.nn.utils.clip_grad_norm_(model.saree_cnn.parameters(), max_norm=1.0)
            ...
            if val_acc > best_val_accuracy:
                ...
                # Save the checkpoint
                checkpoint_dir = '/scratch/data/m23csa016/meesho_data/checkpoints/clipvit_large/ht_val_e40'
                os.makedirs(checkpoint_dir, exist_ok=True)
                
                save_path = os.path.join(checkpoint_dir, f'train_sc_{epoch}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth')
