...
import open_clip
import wandb
from torch.optim.lr_scheduler import CosineAnnealingLR, MultiStepLR, LinearLR, ReduceLROnPlateau
...
def validate_model(model, clip_model, val_loader, criterion, device, logger):
    ...
    with torch.no_grad():
        for images, categories, targets in val_loader:
            ...
            clip_features = clip_model.module.encode_image(images)
...
def setup_logging(log_dir="logs_e40"):
    ...
    log_file = os.path.join(log_dir, f'vith14_gelu_swa_{timestamp}.log')
...
def train_model_with_validation(clip_model, model, train_loader, val_loader, device, train_dataset, 
    ...
    """
    check_path = "/scratch/data/m23csa016/meesho_data/checkpoints/clipvit_large/vith14_quickgelu/vith14_quickgelu_9_trainval_203707.pth"
    ...
    swa_clip_model = torch.optim.swa_utils.AveragedModel(clip_model)

    swa_scheduler = torch.optim.swa_utils.SWALR(optimizer, swa_lr=0.0005)

    logger.info(
        "SWA training with vith14_quickgelu_9_trainval_203707.pth\n"
    )
        ...
        for epoch in epoch_pbar:
            ...
            for batch_idx, (images, categories, targets) in enumerate(train_pbar):
                ...
                try:
                    ...
                    # Configuration
                    accumulation_steps = 1 # Adjust this based on your needs
                    ...
                    if valid_predictions > 0:
                        ...
                        if (batch_idx + 1) % accumulation_steps == 0:
                            ...
                            swa_clip_model.update_parameters(clip_model)
            ...
            torch.optim.swa_utils.update_bn(train_loader, swa_clip_model)

            # Validation phase
            val_loss, val_acc, val_attr_accuracies = validate_model(
                swa_model, swa_clip_model, val_loader, criterion, device, logger
            )

            # scheduler.step()

            # if epoch < 2:  # During warmup period
            ...
            if val_acc > best_val_accuracy:
                ...
                checkpoint = {
                    ...
                    'swa_clip_model_state_dict': swa_clip_model.state_dict(),
                ...
                # Save checkpoints
                checkpoint_dir = '/scratch/data/m23csa016/meesho_data/checkpoints/clipvit_large/vith14_gelu_swa'
                os.makedirs(checkpoint_dir, exist_ok=True)

                save_path = os.path.join(checkpoint_dir, 
                    f'vith14_gelu_swa_{epoch}_trainval_{datetime.now().strftime("%H%M%S")}.pth')
                torch.save(checkpoint, save_path)
                
                # Save metadata
                metadata_path = save_path.replace('.pth', '_metadata.json')
                metadata = {k: v for k, v in checkpoint.items() 
                          if k not in ['model_state_dict', 'clip_model_state_dict', 'optimizer_state_dict', 'swa_model_state_dict', 'swa_clip_model_state_dict']}
...
def main():
    ...
    try:
        ...
        logger.info("Training with 'train.csv'\n")
        ...
        for hidden_dim, dropout_rate, num_hidden_layers in product(
            ...
            for clip_lr, predictor_lr, weight_decay, beta1, beta2 in product(
                ...
                try:
                    ...
                    run_name = f"{batch_size}x{dropout_rate}_swa_finetuning"
                    wandb.init(
                        project="ViT-H-14-Quickgelu",
                        name=run_name,
                        config=config,
                        notes="Training with MultiStepLR(4, 6, 10), SWA, best settings"
                    )
