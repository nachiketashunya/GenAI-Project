...
def train_model_with_validation(model, train_loader, val_loader, device, train_dataset, 
    ...
    optimizer = optim.AdamW([
        ...
        {'params': connector_mlp.parameters(), 'lr': predictor_lr, 'weight_decay': weight_decay}  # Add connector_mlp
    ], betas=(beta1, beta2))
    ...
    try:
        for epoch in epoch_pbar:
            ...
            if val_acc > best_val_accuracy:
                ...
                save_path = os.path.join(checkpoint_dir, f'ht_connect_bgremove_{epoch+1}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth')
