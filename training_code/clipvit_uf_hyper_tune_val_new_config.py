...
def setup_logging(log_dir="/iitjhome/m23csa016/meesho_code/logs_e40"):
    ...
    log_file = os.path.join(log_dir, f'vith14_378_trainval_{timestamp}.log')
...
def train_model_with_validation(clip_model, model, train_loader, val_loader, device, train_dataset, 
    ...
    scheduler = MultiStepLR(optimizer=optimizer, milestones=[1, 2, 3], gamma=0.1)
    ...
    try:
        for epoch in epoch_pbar:
            ...
            #     # Save checkpoints
            #     checkpoint_dir = '/scratch/data/m23csa016/meesho_data/checkpoints/clipvit_large/vith14_378_trainval'
            #     os.makedirs(checkpoint_dir, exist_ok=True)

            #     save_path = os.path.join(checkpoint_dir, 
            #         f'vith14_378_trainval_{epoch}_{datetime.now().strftime("%H%M%S")}.pth')
        ...
        for hidden_dim, dropout_rate, num_hidden_layers in product(
            ...
            for clip_lr, predictor_lr, weight_decay, beta1, beta2 in product(
                ...
                try:
                    ...
                    run_name = f"{hidden_dim}x{num_hidden_layers}_ontraincsv"
                    wandb.init(
                        project="ViT-H-Quickgelu-378",
                        name=run_name,
                        config=config
                    )
